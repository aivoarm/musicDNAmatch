    ...
    return (
        <div className="relative min-h-screen bg-[#080808] overflow-x-hidden">
            ...
            <div className="max-w-6xl mx-auto px-4 md:px-8 pt-24 pb-20 w-full">
                {/* Header */}
                <div className="glass rounded-[2.5rem] p-8 md:p-10 border border-white/25 mb-6 relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-8 opacity-10"><Brain className="h-32 w-32" /></div>
                    <div className="relative z-10">
                        <button onClick={() => router.back()} className="flex items-center gap-2 mono text-[10px] text-white hover:text-[#FF0000] transition-colors uppercase tracking-widest mb-4 font-bold">
                        ...
                        <p className="mono text-[10px] text-white/95 uppercase tracking-[0.4em] font-medium">
                ...
                <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-1">
                    <Filter className="h-3.5 w-3.5 text-white/80 shrink-0" />
                        ...
                        <button key={mode} onClick={() => setFilter(mode)}
                            className={`mono text-[9px] font-black uppercase tracking-widest px-4 py-2 rounded-full border transition-all shrink-0
                                ${filter === mode
                                    ...
                                    : "bg-white/10 border-white/30 text-white hover:text-white hover:border-white/50"}`}>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-4">
                        {loading ? (
                            <div className="glass rounded-[2.5rem] p-16 flex flex-col items-center justify-center">
                                ...
                                <p className="mono text-[10px] text-white/95 uppercase tracking-widest font-bold">Calculating Cosine Distances…</p>
                            </div>
                        ) : filtered.length === 0 ? (
                                ...
                                return (
                                    <motion.div key={match.id || idx}
                                        ...
                                        className={`glass rounded-[2rem] p-6 md:p-8 border transition-all relative overflow-hidden group
                                            ${match.is_mutual ? "ring-2 ring-green-500/40 border-green-500/30" : "border-white/25 hover:border-[#FF0000]/40"}`}>
                                        <div className="flex flex-col sm:flex-row gap-5 items-start sm:items-center">
                                            {/* Avatar + score */}
                                            <div className="relative shrink-0">
                                                <div className="h-16 w-16 md:h-20 md:w-20 rounded-2xl overflow-hidden bg-white/12 ring-2 ring-white/20 group-hover:ring-[#FF0000]/50 transition-all shadow-xl">
                                                    {match.metadata?.images?.[0]?.url
                                                        ? <img src={match.metadata.images[0].url} alt="" className="h-full w-full object-cover" />
                                                        : <div className="h-full w-full flex items-center justify-center text-white font-black text-2xl">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center gap-2 mb-1.5">
                                                    ...
                                                    {match.incoming_signal && !match.is_mutual && (
                                                        <span className="text-[8px] font-black bg-[#FF0000]/15 text-[#FF0000] px-2 py-0.5 rounded-full border border-[#FF0000]/20 uppercase tracking-widest shrink-0 animate-pulse">Signal Received</span>
                                                    )}
                                                </div>
                                                ...
                                                <div className="flex flex-wrap gap-1.5 mb-4">
                                                    {(match.metadata?.top_genres || []).slice(0, 4).map((g: string, i: number) => (
                                                        <span key={g + i} className="text-[8px] bg-white/12 border border-white/30 text-white px-2 py-0.5 rounded-md font-black uppercase tracking-widest shadow-sm">{g}</span>
                                                    ))}
                                                ...
                                                <div className="flex gap-2">
                                                    {match.is_mutual ? (
                                                        ...
                                                        <button onClick={() => setSelectedMatch(match)}
                                                            className="flex items-center gap-2 bg-[#FF0000] text-white font-black py-2.5 px-5 rounded-xl text-[10px] uppercase tracking-widest hover:scale-[1.02] active:scale-95 transition-all shadow-lg animate-pulse">
                                                            <Activity className="h-3.5 w-3.5 shadow-lg" />Respond to Signal
                                                        </button>
                                                    ) : (
                                                        <button onClick={() => !match.has_signal && setSelectedMatch(match)}
                                                            ...
                                                            className={`flex items-center gap-2 font-black py-2.5 px-5 rounded-xl text-[10px] uppercase tracking-widest transition-all
                                                                ${match.has_signal ? "bg-white/15 text-white cursor-not-allowed" : "bg-white text-black hover:scale-[1.02] active:scale-95 shadow-lg"}`}>
                                                            {match.has_signal
                                                                ? <><Activity className="h-3.5 w-3.5 text-white" />Signal Sent</>
                    ...
                    <div className="space-y-5">
                        {/* Active bridges */}
                        <div className="glass rounded-[2rem] p-7 border border-white/25">
                            ...
                            {matches.filter(m => m.is_mutual).length === 0 ? (
                                <div className="p-6 text-center border border-dashed border-white/40 rounded-2xl">
                                    <p className="mono text-[9px] text-white/95 uppercase tracking-widest font-bold">No mutual connections yet</p>
                                </div>
                            ) : (
                                <div className="space-y-2.5">
                                    {matches.filter(m => m.is_mutual).map((m, i) => (
                                        <Link key={m.id || i} href={`/temp-room/${m.bridge_id}`}
                                            className="flex items-center gap-3 p-3.5 rounded-2xl bg-white/10 border border-white/30 hover:bg-white/15 hover:border-green-500/50 transition-all group shadow-sm">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <p className="mono text-[8px] text-white/95 uppercase tracking-widest font-medium">Bridge Active</p>
                                            </div>
                                            <ChevronRight className="h-3.5 w-3.5 text-white group-hover:text-white transition-colors" />
                                        </Link>
                                    ))}
                                </div>
                            )}
                        ...
                        {userDna && (
                            <div className="glass rounded-[2rem] p-7 border border-[#FF0000]/15 bg-[#FF0000]/3">
                                ...
                                <div className="flex items-end justify-between border-b border-white/25 pb-4 mb-4">
                                    ...
                                    <div className="text-right shrink-0">
                                        ...
                                        <p className="mono text-[8px] text-white/95 uppercase tracking-widest font-bold">Coherence</p>
                                    </div>
                                </div>
                                <div className="space-y-4">
                                    <div className="flex flex-wrap gap-1.5">
                                        {(userDna.top_genres || []).slice(0, 4).map((g: string, i: number) => (
                                            <span key={g + i} className="text-[8px] bg-white/12 border border-white/25 text-white/85 px-2 py-1 rounded-md font-black uppercase tracking-widest">{g}</span>
                                        ))}
                                    </div>
                                    <div className="flex flex-wrap gap-1.5">
                                ...
                                <Link href="/profile" className="flex items-center justify-center gap-2 mt-5 border border-white/30 text-white font-black text-[9px] uppercase tracking-widest py-3 rounded-xl transition-all hover:border-white/50 bg-white/5">
                                    View Full Profile <ChevronRight className="h-3 w-3" />
                                </Link>
                                <button onClick={handleRestart} className="relative w-full flex items-center justify-center gap-2 mt-3 border border-[#FF0000]/40 bg-[#FF0000]/15 text-[#FF0000] font-black text-[9px] uppercase tracking-widest py-3 rounded-xl transition-all hover:bg-[#FF0000]/25">
                                    <RotateCcw className="h-3 w-3" /> Start New Signal
                                </button>
                            </div>
                        )}
                    </div>
                ...
                {selectedMatch && (
                    <div className="fixed inset-0 z-[60] flex items-center justify-center p-6">
                        ...
                        <motion.div initial={{ scale: 0.95, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 10 }}
                            className="glass w-full max-w-md rounded-[2.5rem] p-8 md:p-10 relative z-10 border border-white/25">
                            <button onClick={() => setSelectedMatch(null)} className="absolute top-5 right-5 p-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors"><X className="h-4 w-4" /></button>
                            ...
                            {success ? (
                                ...
                                <>
                                    <div className="flex items-center gap-4 mb-6">
                                        <div className="h-12 w-12 rounded-xl bg-white/15 flex items-center justify-center text-white/90 font-black text-lg shadow-inner">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="mono text-[9px] text-white/95 uppercase tracking-widest truncate font-bold">Target: {String(selectedMatch.metadata?.display_name || selectedMatch.display_name || "Anonymous").trim() || "Anonymous"}</p>
                                        </div>
                                    </div>
                                    <form onSubmit={handleInterest} className="space-y-5">
                                        <div>
                                            ...
                                            <div className="relative">
                                                <Mail className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-white/90" />
                                                <input type="email" required value={email} onChange={e => setEmail(e.target.value)}
                                                    placeholder="you@example.com"
                                                    className="w-full bg-white/10 border border-white/30 rounded-xl py-4 pl-11 pr-4 focus:outline-none focus:border-[#FF0000]/80 transition-all mono text-sm text-white placeholder:text-white/70" />
                                            </div>
                                        </div>
                                        <p className="mono text-[9px] text-white/95 leading-relaxed font-bold">
